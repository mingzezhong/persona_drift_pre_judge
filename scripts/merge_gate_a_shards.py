#!/usr/bin/env python3
"""Validate and merge immutable Gate A axis shards."""
...
def main() -> None:
    ...
    axes = list(data["axes"])
    expected_per_axis = (
        len(data["conditions"]) * len(data["topics"]) * len(data["seeds"])
    )
    ...
    for axis in axes:
        shard_dir = output_dir / "shards" / axis
        ...
        if len(trajectories) != expected_per_axis:
            raise ValueError(
                f"{axis} has {len(trajectories)} trajectories, "
                f"expected {expected_per_axis}"
            )
        if len(probes) != expected_per_axis * len(checkpoints):
            raise ValueError(f"{axis} has an unexpected number of probes")
        if any(record["axis"] != axis for record in trajectories + probes):
            raise ValueError(f"axis mismatch in shard {axis}")
        if any(len(record["turns"]) != total_turns for record in trajectories):
            raise ValueError(f"trajectory length mismatch in shard {axis}")
        shard_hashes[axis] = {
