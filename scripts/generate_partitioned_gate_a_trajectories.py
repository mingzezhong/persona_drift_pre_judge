#!/usr/bin/env python3
"""Generate one immutable axis-topic shard of Gate A trajectories."""
...
def select_topic_ids(
    configured_topic_ids: list[str], requested_topic_id: str
) -> list[str]:
    """Select one frozen topic without changing its configured identity or order."""
    if requested_topic_id not in configured_topic_ids:
        raise ValueError(f"topic is not configured: {requested_topic_id}")
...
def main() -> None:
    ...
    topic_ids = select_topic_ids(topic_ids, args.topic)
