#!/bin/bash
# Freeze and submit the token-only OLMo generation-length pilot.
...
for path in "$ROOT/cap256" "$ROOT/cap384" "$ROOT/summary.json" "$ROOT/protocol_files.sha256" "$ROOT/job_ids.txt"; do
  if [[ -e "$path" ]]; then
...
FILES=(
  ...
  docs/olmo_generation_length_pilot_v1_preregistration.md
  data/templates/persona_cross_model_olmo_v1.yaml
  outputs/cross_model_replication/olmo_v1/generation_quality.json
  outputs/cross_model_replication/olmo_v1/merge_summary.json
  outputs/cross_model_replication/olmo_v1/execution_v3_downstream_protocol_files.sha256
  outputs/cross_model_replication/olmo_v1/vectors/persona_vectors.pt
  scripts/generate_partitioned_gate_a_trajectories.py
  scripts/merge_partitioned_gate_a_shards.py
  scripts/validate_generation_length_pilot.py
  scripts/analyze_generation_length_pilot.py
  tests/test_generation_length_pilot.py
  jobs/olmo_generation_length_pilot_v1_validate.pbs
  jobs/olmo_generation_length_pilot_v1_generate.pbs
  jobs/olmo_generation_length_pilot_v1_merge.pbs
  jobs/submit_olmo_generation_length_pilot_v1.sh
)
...
VALIDATE_JOB="$(qsub jobs/olmo_generation_length_pilot_v1_validate.pbs)"
